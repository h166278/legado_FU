...
object AiPromptStore {
    ...
    enum class Prompt(
        ...
        PARAGRAPH_PURIFY(
            ...
            skillId = AiSkillRegistry.SKILL_PARAGRAPH_PURIFY,
            defaultPrompt = PARAGRAPH_PURIFY_SKILL
        ),
        RULE_GENERATE(
            id = "rule_generate",
            key = PreferKey.aiPromptRuleGenerate,
            skillId = AiSkillRegistry.SKILL_CHAPTER_PURIFY,
            defaultPrompt = RULE_GENERATE_SKILL
        )
    }

    fun prompt(type: Prompt): String {
        return AiSkillRegistry.skillPrompt(type.skillId)
            ?: appCtx.getPrefString(type.key, null)
            ?.takeIf { it.isNotBlank() }
            ?: type.defaultPrompt
    }

    fun legacyPrompt(type: Prompt): String? {
